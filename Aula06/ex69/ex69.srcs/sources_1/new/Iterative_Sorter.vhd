...
begin
    ...
    begin
        if rising_edge(clk) then
            ...
            else C_S <= N_S; MyAr <= N_MyAr; counter <= N_counter;
                sorting_completed <= N_sorting_completed; even_odd_switcher <= N_even_odd_switcher;
            end if;
        end if;
    end process;
    ...
    begin
        N_S <= C_S; N_MyAr <= MyAr; N_counter <= counter;
        N_even_odd_switcher <= even_odd_switcher; N_sorting_completed <= sorting_completed;
        case C_S is
            when initial_state => N_sorting_completed <= '0'; N_even_odd_switcher <= '0'; N_counter <= 0;
